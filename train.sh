...
#SBATCH --output=logs/mnv2_phaseft_nomask.out
...
MODEL_NAME="MNv2_phase" # RN50, RN50_nd, RN50_npr, CLIP_vitl14, MNv2, SWIN_t, VGG11  - add '_phase' if using phase layer
...
python -m torch.distributed.launch --nproc_per_node=$NUM_GPU --master_port=29200 train.py \
  -- \
